...
def visualize_hac_with_centroids(X_clean, clean_df, k, perplexity=10):
    ...
    centroids_dict = calculate_event_centroids(X_clean, clean_df['Cluster'].astype(int).values)
    
    # Convert dict to array ordered by cluster ID
    # We use sorted keys to ensure the order matches the cluster IDs
    centroid_matrix = np.array([centroids_dict[i] for i in sorted(centroids_dict.keys())])
    ...
    for cluster_id in sorted(clean_df['Cluster'].unique()):
        ...
        fig.add_trace(go.Scatter(
            x=articles_2d[mask.values, 0], y=articles_2d[mask.values, 1],
            mode='markers', 
