...
def assign_tweets_to_events_by_period(
    tweets_df, news_signatures, start_date, end_date, threshold=0.6
):
    """
...
def plot_tweet_assignment_bars(
    tweets_df, news_signatures, start_date, end_date, threshold=0.55
):
    # 1. Filtrage par date
    mask = (tweets_df["date"] >= start_date) & (tweets_df["date"] <= end_date)
    ...
    colors = [
        "#2ecc71" if sim >= threshold else "#e74c3c"
        for sim in df_period["max_similarity"]
    ]
