    ...
    if event_days:
        ...
        for i in range(1, len(event_days)):
            ...
            if (event_days[i] - prev_date).days > gap_tolerance:
                gt_intervals.append({'start': pd.Timestamp(start_date), 'end': pd.Timestamp(prev_date)})
...
def plot_sp500_ground_truth_simplified(price_df, gt_intervals):
    ...
    fig.add_annotation(
        x='2023-03-10', y=1.02, yref='paper',
        text="Crise SVB", showarrow=False, font=dict(color="#e67e22", size=11)
    )
    ...
    fig.add_annotation(
        x='2023-09-14', y=1.02, yref='paper',
        text="IPO ARM", showarrow=False, font=dict(color="#27ae60", size=11)
    )
...
def generate_alerts_for_period(all_tweets, assigned_tweets, start_date, end_date, alert_threshold_ratio=0.20):
    # Filtrage temporel
    mask_all = (all_tweets['date'] >= start_date) & (all_tweets['date'] <= end_date)
    mask_assigned = (assigned_tweets['date'] >= start_date) & (assigned_tweets['date'] <= end_date)
    
    # Agrégation
    daily_total = all_tweets[mask_all].groupby(all_tweets['date'].dt.date).size()
    daily_assigned = assigned_tweets[mask_assigned].groupby(assigned_tweets['date'].dt.date).size()
...
def evaluate_pipeline_for_period(alert_df, gt_intervals, start_date, end_date):
    ...
    alerts_in_period = alert_df[(alert_df['is_alert'] == True) & 
                                (pd.to_datetime(alert_df['date']) >= start_obs) & 
                                (pd.to_datetime(alert_df['date']) <= end_obs)]['date'].tolist()
    ...
    relevant_gt = [
        event for event in gt_intervals 
        if (event['start'] <= end_obs) and (event['end'] >= start_obs)
    ]
    
    if not relevant_gt:
        return "Aucun événement de marché (GT) sur cette période pour le calcul du Recall."
...
def plot_social_heat_curve(alert_df, gt_intervals, threshold):
    fig = px.line(alert_df, x='date', y='ratio', 
